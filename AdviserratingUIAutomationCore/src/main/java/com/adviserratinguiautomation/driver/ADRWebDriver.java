...
public class ADRWebDriver {
    ...
    private static ADRWebDriver adrWebDriver = null;

    ADRWebdriverFactory adrWebdriverFactory = new ADRWebdriverFactory();
    ...
    protected ADRWebDriver() {
        log.info("Entered the ADRWebDriver method");
        cleanUp();
    	adrWebdriverFactory.init();
        browser = adrWebdriverFactory.getBrowser();
        seleniumWebDriver = adrWebdriverFactory.getADRWebDriverInstance();
        if (seleniumWebDriver != null) {
            seleniumWebDriver.manage().window().maximize();
        }
        log.info("Exited the ADRWebDriver method");
    }

    protected ADRWebDriver(String browser) {
        log.info("Entered the ADRWebDriver method with browser name: " + browser);
        adrWebdriverFactory.init();
        seleniumWebDriver = adrWebdriverFactory.getBrowserInstance(browser);
        seleniumWebDriver.manage().window().maximize();
        log.info("Exited the ADRWebDriver method with browser name");
    }
    ...
    public static ADRWebDriver getADRWebDriverInstance() {
        log.info("Entered the getADRWebDriverInstance method");
        if (adrWebDriver == null) {
        	adrWebDriver = new ADRWebDriver();
        }
        log.info("Exited the getADRWebDriverInstance method");
        return adrWebDriver;
    }
    ...
    public void resetNewBrowser(String browser) {
        ...
        adrWebDriver = new ADRWebDriver(browser);
...
public void cleanUp() {
	try {
		...
		System.out.println("Entered the cleanUp method");
